...
// Dynamic import for VoiceChat to reduce initial bundle size
const VoiceChat = dynamic(
  () => import("@/components/VoiceChat").then((mod) => ({ default: mod.VoiceChat })),
  {
    loading: () => (
      <div className="flex items-center justify-center h-screen">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-500 mx-auto mb-4"></div>
          <p className="text-gray-600">음성 채팅 로딩 중...</p>
        </div>
      </div>
    ),
    ssr: false,
...
export default function VoiceChatPage() {
  ...
  if (credits && credits.remainingMinutes <= 0 && credits.subscriptionTier !== "pro" && credits.subscriptionTier !== "pro+") {
    return (
      <div className="flex items-center justify-center h-screen bg-gradient-to-br from-primary-50 via-white to-secondary-50 text-gray-900">
        <div className="text-center max-w-md px-6">
          ...
          <div className="space-y-3">
            <button
              ...
              className="w-full bg-gradient-to-r from-primary-500 to-secondary-500 hover:from-primary-600 hover:to-secondary-600 px-6 py-3 rounded-lg font-bold"
            >
              Pro로 업그레이드 (무제한!)
            </button>
            {credits.subscriptionTier === "free+" && (
              <button
                ...
                className="w-full bg-white hover:bg-gray-50 text-gray-700 border-2 border-gray-300 px-6 py-3 rounded-lg"
              >
                추가 60분 구매 ($2)
              </button>
            )}
      ...
      <div className="flex h-screen bg-gradient-to-br from-primary-50 via-white to-secondary-50">
          ...
          <div className="text-center text-gray-900 px-6">
            ...
            <div className="space-y-4">
              <button
                ...
                className="w-full max-w-xs bg-gradient-to-r from-primary-500 to-secondary-500 hover:from-primary-600 hover:to-secondary-600 px-6 py-3 rounded-lg font-bold"
              >
                새 대화 시작
              </button>
              {sessions.length > 0 && (
                <button
                  ...
                  className="w-full max-w-xs bg-white hover:bg-gray-50 text-gray-700 border-2 border-gray-300 px-6 py-3 rounded-lg"
                >
        ...
        {showSidebar && (
          <div className="fixed inset-0 z-50 lg:hidden">
            ...
            />
            <div className="absolute left-0 top-0 bottom-0 w-64 bg-white dark:bg-gradient-to-br from-primary-50 via-white to-secondary-50 transform transition-transform">
              <Sidebar
                sessions={sessions}
                currentSessionId=""
                onSessionSelect={handleSelectSession}
